...
export function validArticle(a){
 ...
 const language=a.language==='th'||a.language==='en'?a.language:(/[ก-๙]/.test(a.title)?'th':'en');
 const market=a.market==='thailand'?'thailand':'global';
 return {id:a.id.slice(0,100),title:a.title.slice(0,350),url:canonical.slice(0,2000),sourceId:a.sourceId.slice(0,100),source:a.source.slice(0,100),category:a.category,publishedAt:typeof a.publishedAt==='string'&&Number.isFinite(Date.parse(a.publishedAt))?a.publishedAt:null,regions:Array.isArray(a.regions)&&a.regions.some(r=>regions.includes(r))?[...new Set(a.regions.filter(r=>regions.includes(r)))]:inferRegions(a.title),language,market};
}
