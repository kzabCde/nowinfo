...
export function mergeNews(fresh,previous,now=Date.now()){
 ...
 for(const a of [...fresh.articles,...(previous?.articles||[])]){const headline=a.title.toLowerCase().replace(/[^\p{L}\p{N}]/gu,'');if(seen.has(a.id)||seen.has(a.url)||seen.has(headline))continue;if(a.publishedAt&&Date.parse(a.publishedAt)<now-7*86400000)continue;seen.add(a.id);seen.add(a.url);seen.add(headline);articles.push(a);}
