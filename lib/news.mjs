...
function decodeNumericEntities(value) {
  return value.replace(/&#(\d+);/g,(match,digits)=>{const code=Number(digits);return Number.isInteger(code)&&code>=0&&code<=0x10ffff&&!(code>=0xd800&&code<=0xdfff)?String.fromCodePoint(code):match;}).replace(/&#x([0-9a-f]+);/gi,(match,hex)=>{const code=Number.parseInt(hex,16);return Number.isInteger(code)&&code>=0&&code<=0x10ffff&&!(code>=0xd800&&code<=0xdfff)?String.fromCodePoint(code):match;});
}
export function plain(value) { return decodeNumericEntities(String(value ?? '').replace(/<[^>]*>/g,' ').replace(/&nbsp;/gi,' ')).replace(/\s+/g,' ').trim(); }
export function categorize(title, fallback) {
  if(/\b(oil|gas|opec|energy|petroleum|fuel|electricity)\b/i.test(title)||/(น้ำมัน|ก๊าซ|พลังงาน|เชื้อเพลิง|ค่าไฟ|โรงไฟฟ้า|ระบบไฟฟ้า|ไฟฟ้าดับ|ไฟฟ้าขัดข้อง)/.test(title)) return 'energy';
  if(/\b(climate|flood|hurricane|wildfire|emission|warming|drought|earthquake|pollution)\b/i.test(title)||/(น้ำท่วม|ท่วมฉับพลัน|น้ำป่า|ฝนตกหนัก|ฝนถล่ม|พายุ|คลื่นสูง|ไฟป่า|ภัยแล้ง|แผ่นดินไหว|โลกร้อน|สิ่งแวดล้อม|มลพิษ|ฝุ่น|พีเอ็ม\s?2\.5|PM\s?2\.5)/i.test(title)) return 'climate';
