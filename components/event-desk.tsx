...
import type {Article,NewsEvent,Interests} from '@/lib/types';
...
import {hasInterests,interestMatches} from '@/lib/workspace.mjs';
import {countryOptions} from '@/lib/focus.mjs';

type Props={mode:'events'|'following';interests:Interests;onInterests:(value:Interests)=>void;articles:Article[];lang:'th'|'en';loading:boolean;saved:Article[];onBookmark:(article:Article)=>void;onOpen:(article:Article)=>void};
export default function EventDesk({articles,lang,loading,saved,onBookmark,onOpen,mode,interests,onInterests}:Props) {
  const [focus,setFocus]=useState(mode==='following'?'all':'relevant');
  const [region,setRegion]=useState('all');
  const [query,setQuery]=useState('');
  const [limit,setLimit]=useState(18);
  const t=(th:string,en:string)=>lang==='th'?th:en;
  const events=useMemo(()=>clusterEvents(articles) as NewsEvent[],[articles]);
  const filtered=useMemo(()=>events.filter(event=>(mode!=='following'||event.articles.some(article=>interestMatches(article,interests).matched))&&event.articles.some(article=>matchesFocus(article,focus))&&(region==='all'||event.regions.includes(region))&&event.articles.some(article=>`${article.title} ${article.source}`.toLowerCase().includes(query.trim().toLowerCase()))),[events,focus,region,query,mode,interests]);
  ...
  return <section className="event-desk" aria-label={t('เหตุการณ์สำคัญ','Event desk')}>
    {mode==='following'&&<InterestsEditor value={interests} onChange={onInterests} lang={lang}/>}
    ...
    {!filtered.length?<div className="empty-state"><h3>{loading&&!articles.length?t('กำลังรวบรวมข่าว','Gathering headlines'):mode==='following'&&!hasInterests(interests)?t('เริ่มจากเลือกเรื่องที่สนใจด้านบน','Choose an interest above to get started'):t('ยังไม่พบเหตุการณ์ตามตัวกรอง','No matching stories')}</h3><p>{t('ลองเปลี่ยนประเด็น ภูมิภาค หรือคำค้น','Try another topic, region or search.')}</p><button className="text-button" onClick={()=>{setFocus('all');setRegion('all');setQuery('');}}>{t('ดูข่าวทั้งหมด','Show all headlines')}</button></div>:<div className="event-grid">{filtered.slice(0,limit).map(event=><article className="event-card panel" key={event.id}>
      ...
      {mode==='following'&&<p className="match-reason">{t('ตรงกับที่คุณติดตาม','Matches your interests')}: {[...new Set(event.articles.flatMap(article=>{const match=interestMatches(article,interests);return [...match.keywords,...match.topics.map(id=>focusTopics.find(topic=>topic.id===id)?.[lang]),...match.countries.map(id=>countryOptions.find(country=>country.id===id)?.[lang])];}))].join(', ')}</p>}
